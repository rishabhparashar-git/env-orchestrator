...
import { Import, X, CheckSquare, Square, AlertTriangle, ListPlus } from "lucide-react";
...
export function ImportProjectModal({
        ...
        const handleImport = () => {
                ...
                const overlappingCount = keysToImport.filter(k => existingKeyNames.has(k.name)).length;
                if (overlappingCount > 0) {
                        const confirmed = window.confirm(`You selected ${overlappingCount} variable(s) that already exist in your project.\n\nImporting them will OVERWRITE their configuration (description and overrides), but your stored values will remain safely intact.\n\nDo you want to proceed?`);
                        if (!confirmed) return;
                }
        ...
        const displayKeys = sourceSchema?.keys.filter(k => {
                ...
                <div className="fixed inset-0 z-50 bg-background/80 backdrop-blur-sm flex items-center justify-center p-4">
                        <div className="bg-card w-full max-w-5xl max-h-[90vh] overflow-hidden rounded-xl border border-border shadow-2xl flex flex-col animate-in zoom-in-95 duration-200">
                                ...
                                <div className="flex items-center justify-between p-6 border-b border-border bg-muted/30">
                                        <div>
                                                <h2 className="text-2xl font-bold flex items-center gap-2"><Import className="w-6 h-6 text-primary" /> Import Schema</h2>
                                                <p className="text-muted-foreground text-sm">Select a project to import environment variables and override configurations.</p>
                                        </div>
                                ...
                                <div className="flex-1 overflow-y-auto p-6 space-y-8">
                                                ...
                                                <div className="flex flex-col sm:flex-row gap-6 sm:items-center justify-between">
                                                        ...
                                                        {sourceSchema && (
                                                                <div className="flex items-center gap-2 bg-muted/30 p-2 rounded-md border border-border">
                                                                        ...
                                                                        />
                                                                        <Label htmlFor="show-existing" className="cursor-pointer text-sm">Include Variables present in this Project</Label>
                                                                </div>
                                                        )}
                                                </div>
                                        </div>
                                                ...
                                                <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4">
                                                        ...
                                                        <div className="glass-panel p-4 rounded-lg flex flex-col md:flex-row md:items-center justify-between gap-4 sticky top-0 z-10 shadow-sm">
                                                                <div className="flex flex-wrap items-center gap-2">
                                                                        <Button variant="outline" size="sm" onClick={() => handleToggleSelectAll(true)}>
                                                                        ...
                                                                        <Button variant="outline" size="sm" onClick={() => handleToggleSelectAll(false)}>
                                                                                <Square className="w-4 h-4 mr-2" /> Deselect All
                                                                        </Button>
                                                                        <span className="text-sm font-semibold text-primary ml-2 uppercase tracking-wider">{selectedCount} Selected</span>
                                                                </div>

                                                                <div className="flex flex-wrap items-center gap-4 border-t md:border-none pt-4 md:pt-0 border-border">
                                                                        <div className="flex items-center gap-2">
                                                                                <Label className="text-xs whitespace-nowrap">Add Override:</Label>
                                                                                <Select onValueChange={handleApplyGlobalOverride} value="">
                                                                                        ...
                                                                                        <SelectContent>
                                                                                                {overrides.map(ov => <SelectItem key={ov.id} value={ov.id}>{ov.name}</SelectItem>)}
                                                                                        </SelectContent>
                                                                                </Select>
                                                                        </div>
                                                                        <div className="flex items-center gap-2">
                                                                                <Label className="text-xs whitespace-nowrap">Remove Override:</Label>
                                                        ...
                                                        <div className="grid grid-cols-1 gap-4">
                                                                {displayKeys.map(k => {
                                                                        ...
                                                                        return (
                                                                                <div key={k.id} className={`p-4 rounded-lg border transition-colors ${isSelected ? 'border-primary bg-primary/5 shadow-sm' : 'border-border bg-black/5 dark:bg-white/5 opacity-60'} ${isExisting && isSelected ? 'border-yellow-500/50 bg-yellow-500/5' : ''}`}>
                                                                                        <div className="flex items-start gap-4">
                                                                                                <div className="pt-1">
                                                                                                        <Switch
                                                                                                                checked={isSelected}
                                                                                                                onCheckedChange={(c) => setSelectedKeys(prev => ({ ...prev, [k.id]: c }))}
                                                                                                        />
                                                                                                </div>
                                                                                                <div className="flex-1 space-y-4">
                                                                                                        <div>
                                                                                                                <div className="flex items-center gap-3 mb-1">
                                                                                                                        <h4 className="font-mono font-bold text-lg">{k.name}</h4>
                                                                                                                        {isExisting && (
                                                                                                                                <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded text-[10px] font-medium bg-yellow-500/20 text-yellow-600 dark:text-yellow-400 uppercase tracking-widest">
                                                                                                                                        <AlertTriangle className="w-3 h-3" /> Already in Project
                                                                                                                                </span>
                                                                                                                        )}
                                                                                                                </div>
                                                                                                                <p className="text-sm text-muted-foreground">{k.description || "No description provided."}</p>

                                                                                                                {isExisting && isSelected && (
                                                                                                                        <p className="text-xs text-yellow-600 dark:text-yellow-400 mt-2 font-medium">
                                                                                                                                Importing this will overwrite your current description and overrides for this variable. Assigned values will not be lost.
                                                                                                                        </p>
                                                                                                                )}
                                                                                                        </div>
                                                                                                        ...
                                                                                                        {isSelected && overrides.length > 0 && (
                                                                                                                <div className="bg-background/50 p-3 rounded-md space-y-2 border border-border/50">
                                                                                                                        <Label className="text-xs text-muted-foreground uppercase">Overrides for this variable:</Label>
                                                                                                                        <div className="flex flex-wrap gap-4">
                                                                                                                                {overrides.map(ov => {
                                                                                                                                        const isOvAttached = currentOverrides.includes(ov.id);
                                                                                                                                        return (
                                                                                                                                                <Label key={ov.id} className="flex items-center gap-2 cursor-pointer font-normal text-sm p-1">
                                                                                                                                                        <input
                                                                                                                                                                type="checkbox"
                                                                                                                                                                ...
                                                                                                                                                                onChange={(e) => {
                                                                                                                                                                        setKeyOverrides(prev => {
                                                                                                                                                                                ...
                                                                                                                                                                                if (e.target.checked) return { ...prev, [k.id]: [...current, ov.id] };
                                                                                                                                                                                return { ...prev, [k.id]: current.filter(id => id !== ov.id) };
                                                                                                                                                                        });
                                                                                                                                                                }}
                                                                                                                                                        />
                                                                                                                                                        {ov.name}
                                                                ...
                                                                {displayKeys.length === 0 && sourceSchema.keys.length > 0 && (
                                                                        <div className="text-center p-8 text-muted-foreground border border-dashed rounded-xl">
                                                                                All variables in the source project already exist in your current project.<br />
                                                                                Toggle &quot;Include Variables present in this Project&quot; to see them.
                                                                        </div>
                                                                )}
                                                        </div>
                                        ...
                                        {sourceSchema && sourceSchema.keys.length === 0 && (
                                                <div className="text-center p-8 text-muted-foreground">This project has no environment variables configured.</div>
                                        )}
                                ...
                                <div className="p-4 border-t border-border bg-muted/30 flex justify-end gap-4">
                                        ...
                                        <Button onClick={handleImport} disabled={!sourceSchema || selectedCount === 0} className="gap-2">
