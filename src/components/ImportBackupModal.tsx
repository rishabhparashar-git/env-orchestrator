  ...
  useEffect(() => {
    ...
    try {
      ...
      if (parsed.encrypted === true && !parsed.systemPasswordVersion) {
        ...
        try {
          extract = JSON.parse(
            decodeURIComponent(escape(atob(parsed.payload))),
          );
        } catch {
      ...
      <div className="fixed inset-0 z-50 bg-background/80 backdrop-blur-sm flex items-center justify-center p-4">
        <div className="bg-card w-full max-w-lg p-6 rounded-xl border border-border shadow-2xl flex flex-col items-center text-center">
          ...
          <p className="text-muted-foreground mb-6">
            The uploaded file could not be parsed or decrypted by the system.
          </p>
      ...
      <div className="bg-card w-full max-w-lg overflow-hidden rounded-xl border border-border shadow-2xl flex flex-col animate-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center justify-between p-6 border-b border-border bg-muted/30">
          <div>
            ...
            <p className="text-muted-foreground text-sm mt-1">
              Review the backup attributes before restoring.
            </p>
          </div>
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="bg-black/5 dark:bg-white/5 border border-border p-5 rounded-xl space-y-4">
            ...
            <div>
              <Label className="text-[10px] uppercase font-bold text-muted-foreground mb-1 block tracking-widest">
                Description
              </Label>
            ...
            {metadata.tags && metadata.tags.length > 0 && (
              <div className="flex flex-wrap gap-2 pt-2">
                {metadata.tags.map((t: string) => (
                  <span
                    key={t}
                    className="px-2.5 py-1 bg-primary/10 border border-primary/20 text-primary font-semibold text-xs rounded-md shadow-sm"
                  >
                    {t}
                  </span>
                ))}
          ...
          {needsPassword ? (
            <div className="space-y-3 pt-2">
              <div className="flex items-center gap-2 text-yellow-600 dark:text-yellow-500 font-medium">
                <ShieldCheck className="w-4 h-4" /> This configuration is
            ...
            <div className="space-y-3 pt-2">
              <div className="flex items-center gap-2 text-green-600 dark:text-green-500 font-medium text-sm">
                <FileJson className="w-4 h-4" /> This file does not require a
                password to be imported.
              </div>
            </div>
          )}
          ...
          <div className="bg-destructive/10 border border-destructive/20 p-4 rounded-lg flex items-start gap-3 mt-6">
            ...
            <div className="text-sm">
              <strong className="text-destructive block mb-1">
                Warning: Destructive Action
              </strong>
              Restoring from this backup will completely overwrite your current
              browser Configuration State (Projects, Variables, Overrides, etc).
            </div>
          </div>
        </div>
