...
import { Copy, Download, Check, FileJson, FileText, BookmarkPlus } from "lucide-react";
...
export function ExportPreviewModal({
        ...
        const handleCopy = async () => {
                try {
                        await navigator.clipboard.writeText(activeTab === "env" ? envText : jsonText);
                        setCopied(true);
                                        ...
                                        <div className="bg-primary/5 border border-primary/20 rounded-xl p-4 flex flex-col sm:flex-row justify-between items-center gap-4">
                                                {existingPreset ? (
                                                                ...
                                                                <div className="flex items-center gap-3">
                                                                        ...
                                                                        <div>
                                                                                ...
                                                                                <p className="text-xs text-muted-foreground">
                                                                                        This configuration matches your saved <strong className="text-green-500">{existingPreset.name}</strong> preset.
                                                                                </p>
                                                                        </div>
                                                                </div>
                                                        </div>
                                                        ...
                                                        <div className="flex items-center gap-2 w-full">
                                                                ...
                                                                <Button variant="ghost" size="sm" onClick={() => setIsSavingPreset(false)}>Cancel</Button>
                                                        </div>
                                                ) : (
                                                        <>
                                                                <div className="flex items-center gap-3">
                                                                        ...
                                                                        <div>
                                                                                <h4 className="font-semibold text-sm text-foreground">Save Configuration Preset</h4>
                                                                                <p className="text-xs text-muted-foreground">Save these specific override options to recreate this environment instantly later.</p>
                                                                        </div>
                                                                </div>
                                                                <Button variant="outline" size="sm" onClick={() => setIsSavingPreset(true)} className="whitespace-nowrap shrink-0 border-primary/30 text-primary hover:bg-primary/20">
                                                                        Create Preset
                                                                </Button>
                                                        </>
                                                )}
