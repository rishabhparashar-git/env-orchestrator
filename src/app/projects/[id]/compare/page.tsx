...
import { ArrowLeft, ArrowRightLeft, Plus, MoveRight, MoveLeft, Eye, EyeOff, Save } from "lucide-react";
...
export default function EnvCompare() {
        ...
        const { projects, schemas, overrides, presets, values, setValue, privacyMode, setPrivacyMode } = useEnvStore();
        ...
        const [rightSelections, setRightSelections] = useState<Record<string, string>>({});
        ...
        const renderConfigSelector = (
                ...
                <div className="glass-panel p-6 rounded-xl space-y-4">
                        ...
                        <div>
                                <label className="text-xs text-muted-foreground uppercase mb-1 block">Load Preset</label>
                        ...
                        <div className="pt-2">
                                <label className="text-xs text-muted-foreground uppercase mb-2 block">Active Overrides</label>
                                <div className="space-y-3">
                                        {overrides.map(ov => (
                                                <div key={ov.id} className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                                                        <span className="text-sm border-l-2 border-primary/50 pl-2">{ov.name}</span>
                                                        <Select
                                                        ...
                                                        >
                                                                ...
                                                                <SelectContent>
                                                                        <SelectItem value="none" className="text-muted-foreground">Global/Base</SelectItem>
                                                                        {ov.options.map(opt => (
        ...
        const isVisible = (keyName: string) => {
                if (privacyMode as unknown === false || privacyMode === "show") return true;
                if (privacyMode as unknown === true || privacyMode === "hide") return false;
        ...
        return (
                <div className="max-w-7xl mx-auto p-6 md:p-12">
                        <header className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
                                <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                                ...
                                                <p className="text-muted-foreground">Compare and synchronize overrides.</p>
                                        </div>
                                </div>
                        ...
                        <div className="grid md:grid-cols-2 gap-6 mb-8">
                                {renderConfigSelector("Left Environment", leftPresetId, handleLeftPresetChange, leftSelections, setLeftSelections)}
                                {renderConfigSelector("Right Environment", rightPresetId, handleRightPresetChange, rightSelections, setRightSelections)}
                        </div>

                        {/* Diff Table */}
                        <div className="glass-panel rounded-xl overflow-hidden border border-border">
                                <div className="bg-black/20 dark:bg-white/5 p-4 flex items-center justify-between border-b border-border">
                                        <h3 className="font-bold flex items-center gap-2">Variables Matrix</h3>
                                        <div className="flex items-center gap-4 text-xs font-mono">
                                                <span className="flex items-center gap-1"><span className="w-3 h-3 rounded-full bg-green-500/20 border border-green-500"></span> Missing Right</span>
                                                <span className="flex items-center gap-1"><span className="w-3 h-3 rounded-full bg-blue-500/20 border border-blue-500"></span> Missing Left</span>
                                                <span className="flex items-center gap-1"><span className="w-3 h-3 rounded-full bg-yellow-500/20 border border-yellow-500"></span> Mismatch</span>
                                        </div>
                                </div>

                                <div className="divide-y divide-border min-w-[800px] overflow-x-auto">
                                        {diffs.map(diff => (
                                                <div key={diff.keyId} className="grid grid-cols-12 items-center p-4 hover:bg-black/5 dark:hover:bg-white/5 transition-colors gap-4">

                                                        {/* Key Name */}
                                                        <div className="col-span-3 font-mono text-sm break-all font-semibold flex items-center justify-between group">
                                                                <span className={`px-2 py-1 rounded-md ${diff.diffType === "missing-right" ? "bg-green-500/10 text-green-500" :
                                                                                diff.diffType === "missing-left" ? "bg-blue-500/10 text-blue-500" :
                                                                                        diff.diffType === "different" ? "bg-yellow-500/10 text-yellow-500" :
                                                                                                "text-foreground"
                                                                        }`}>
                                                                        {diff.keyName}
                                                                </span>
                                                                {(privacyMode === "partial" || privacyMode as unknown === "partial") && (
                                                                        <Button variant="ghost" size="icon" className="h-6 w-6 opacity-0 group-hover:opacity-100" onClick={() => toggleReveal(diff.keyName)}>
                                                                                {revealedIds[diff.keyName] ? <EyeOff className="w-3 h-3" /> : <Eye className="w-3 h-3" />}
                                                                        </Button>
                                                                )}
                                                        </div>
                                                        ...
                                                        <div className="col-span-1 flex flex-col items-center justify-center gap-1 px-2">
                                                                {diff.diffType !== "equal" ? (
                                                                        <>
                                                                                <Button size="icon" variant="ghost" className="h-6 w-6 rounded-full hover:bg-primary/20 hover:text-primary" onClick={() => handleSyncToRight(diff)} title="Sync Left to Right">
                                                                                        <MoveRight className="w-3 h-3" />
                                                                                </Button>
                                                                                <Button size="icon" variant="ghost" className="h-6 w-6 rounded-full hover:bg-primary/20 hover:text-primary" onClick={() => handleSyncToLeft(diff)} title="Sync Right to Left">
                                                                                        <MoveLeft className="w-3 h-3" />
                                                                                </Button>
                                                                        </>
                                                                ) : (
                                                                        <span className="text-muted-foreground/50"><ArrowRightLeft className="w-4 h-4" /></span>
                                                                )}
                                                        </div>
