...
export default function EnvMatrixForm() {
  ...
  const { projects, schemas, overrides, presets, deletePreset, values, setValue, privacyMode, setPrivacyMode } = useEnvStore();
  ...
  const [exportSelections, setExportSelections] = useState<Record<string, string>>({}); // { overrideId: option }
  ...
  const getInputType = (groupId: string, inputId: string) => {
    const mode = privacyMode as unknown === false ? "show" : (privacyMode as unknown === true ? "hide" : privacyMode);
  ...
  const handleValueChange = (keyId: string, overrideId: string, option: string, val: string) => {
  ...
  const getMappedSelections = (presetToImport: { selections: Record<string, string> }) => {
    const newSelections: Record<string, string> = {};
    Object.entries(presetToImport.selections).forEach(([foreignOverrideId, foreignOptionValue]) => {
      const localOverrideExists = overrides.find(o => o.id === foreignOverrideId);
      if (localOverrideExists && localOverrideExists.options.includes(foreignOptionValue)) {
  ...
  const handleImportPreset = (presetToImport: { name: string, selections: Record<string, string> }) => {
    ...
    return schema.keys.map(k => (
      <div key={k.id} className="glass-panel p-6 rounded-xl space-y-4 mb-6">
        <div className="flex items-start justify-between">
          <div>
            <h3 className="text-xl font-bold group font-mono text-primary flex items-center gap-2 cursor-pointer hover:underline decoration-primary underline-offset-4" onClick={() => copyToClipboard(k.name, `name_${k.id}`)}
              title="Click to copy key name"><span>{k.name}</span> {copiedStates[`name_${k.id}`] ? (
          ...
          {(privacyMode === "partial" || privacyMode as unknown === "partial") && (
            <Button variant="ghost" size="icon" onClick={() => toggleGroupPrivacy(k.id)} title="Toggle Group Visibility">
              {revealedGroups[k.id] ? <EyeOff className="w-5 h-5 text-muted-foreground" /> : <Eye className="w-5 h-5 text-muted-foreground" />}
            </Button>
          )}
        </div>
          ...
          <div>
            ...
            <div className="relative">
              <Input
                ...
                onChange={e => handleValueChange(k.id, "global", "default", e.target.value)}
              ...
              <button
                ...
                onClick={() => copyToClipboard(getHashValue(k.id, "global", "default"), `val_${k.id}_global_default`)}
                title="Copy value"
              >
                {copiedStates[`val_${k.id}_global_default`] ? <Check className="w-4 h-4 text-green-500" /> : <Copy className="w-4 h-4" />}
              </button>
            </div>
          </div>
        ) : (
          <div className="space-y-6">
            <div className="border-b border-border pb-4">
              ...
              <div className="relative mt-2">
                ...
                <button
                  className="absolute right-2 top-1/2 -translate-y-1/2 p-2 hover:bg-muted rounded-md text-muted-foreground focus:outline-none"
                  onClick={() => copyToClipboard(getHashValue(k.id, "global", "default"), `val_${k.id}_global_default_nested`)}
                  title="Copy value"
                >
                  {copiedStates[`val_${k.id}_global_default_nested`] ? <Check className="w-4 h-4 text-green-500" /> : <Copy className="w-4 h-4" />}
                </button>
              </div>
            </div>
              ...
              return (
                <div key={ov.id} className="pl-4 border-l-2 border-primary/50">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {ov.options.map(opt => (
                      <div key={opt}>
                        <Label className="text-xs text-muted-foreground uppercase">{opt}</Label>
                        <div className="relative mt-1">
                          <Input
                            type={getInputType(k.id, `${k.id}_${ov.id}_${opt}`)}
                            value={getHashValue(k.id, ov.id, opt)}
                            onChange={e => handleValueChange(k.id, ov.id, opt, e.target.value)}
                          ...
                          <button
                            className="absolute right-2 top-1/2 -translate-y-1/2 p-2 hover:bg-muted rounded-md text-muted-foreground focus:outline-none"
                            onClick={() => copyToClipboard(getHashValue(k.id, ov.id, opt), `val_${k.id}_${ov.id}_${opt}`)}
                            title="Copy value"
                          >
                            {copiedStates[`val_${k.id}_${ov.id}_${opt}`] ? <Check className="w-4 h-4 text-green-500" /> : <Copy className="w-4 h-4" />}
                          </button>
                        </div>
                      </div>
                    ))}
  ...
  const renderByOverride = () => {
    ...
    const keysUsingOverride = schema.keys.filter(k => k.overrides.includes(ov.id));

    if (keysUsingOverride.length === 0) {
      return <p className="text-muted-foreground p-6 text-center">No keys in this project use the {ov.name} override.</p>;
    }

    return ov.options.map(opt => (
      <div key={opt} className="glass-panel p-6 rounded-xl mb-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-xl font-bold bg-primary/20 text-primary inline-block px-3 py-1 rounded-md uppercase">{opt}</h3>
          {(privacyMode === "partial" || privacyMode as unknown === "partial") && (
            <Button variant="ghost" size="icon" onClick={() => toggleGroupPrivacy(opt)} title="Toggle Group Visibility">
              {revealedGroups[opt] ? <EyeOff className="w-5 h-5 text-muted-foreground" /> : <Eye className="w-5 h-5 text-muted-foreground" />}
            </Button>
          )}
        </div>
        <div className="space-y-4">
          {keysUsingOverride.map(k => (
            <div key={k.id} className="p-4 border border-border rounded-lg bg-black/5 dark:bg-white/5 space-y-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <Label className="text-xs text-muted-foreground">Global Base</Label>
                  <div className="relative mt-1">
                    <Input
                      type={getInputType(opt, `${k.id}_global_default_${opt}`)}
                      value={getHashValue(k.id, "global", "default")}
                      onChange={e => handleValueChange(k.id, "global", "default", e.target.value)}
                      onFocus={() => setFocusedInput(`${k.id}_global_default_${opt}`)}
                      onBlur={() => setFocusedInput(null)}
                      placeholder={`Base value`}
                      className="pr-10"
                    />
                    <button
                      className="absolute right-2 top-1/2 -translate-y-1/2 p-2 hover:bg-muted rounded-md text-muted-foreground focus:outline-none"
                      onClick={() => copyToClipboard(getHashValue(k.id, "global", "default"), `val_ov_${k.id}_global_default`)}
                      title="Copy value"
                    >
                      {copiedStates[`val_ov_${k.id}_global_default`] ? <Check className="w-4 h-4 text-green-500" /> : <Copy className="w-4 h-4" />}
                    </button>
                  </div>
                </div>
                <div>
                  <Label className="text-xs text-primary uppercase">{opt} Override</Label>
                  <div className="relative mt-1">
                    <Input
                    ...
                    <button
                      className="absolute right-2 top-1/2 -translate-y-1/2 p-2 hover:bg-muted rounded-md text-muted-foreground focus:outline-none"
                      onClick={() => copyToClipboard(getHashValue(k.id, ov.id, opt), `val_ov_${k.id}_${ov.id}_${opt}`)}
                      title="Copy value"
                    >
                      {copiedStates[`val_ov_${k.id}_${ov.id}_${opt}`] ? <Check className="w-4 h-4 text-green-500" /> : <Copy className="w-4 h-4" />}
                    </button>
                  </div>
                </div>
              </div>
  ...
  const handleGenerateEnv = () => {
    ...
    outputEnv += `# Overrides: ${Object.entries(exportSelections).map(([id, opt]) => {
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground">Fill environment variables and resolve to .env file.</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          >
            ...
            <SelectContent>
              <SelectItem value="show"><div className="flex items-center"><Eye className="w-4 h-4 mr-2 text-primary" /> Show All</div></SelectItem>
              <SelectItem value="hide"><div className="flex items-center"><EyeOff className="w-4 h-4 mr-2 text-destructive" /> Hide All</div></SelectItem>
              <SelectItem value="partial"><div className="flex items-center"><Eye className="w-4 h-4 mr-2 opacity-50" /> Partial Show</div></SelectItem>
            </SelectContent>
          </Select>
        </div>
      </header>
      ...
      <div className="grid lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="glass-panel p-6 rounded-xl sticky top-6">
            <h3 className="text-xl font-bold mb-4 flex items-center"><Download className="w-5 h-5 mr-2 text-primary" /> Export Generator</h3>
            <p className="text-sm text-muted-foreground mb-6">Select your target environment cascade to compute the final .env file.</p>

            <div className="space-y-4 mb-8">
              {overrides.map(ov => (
                <div key={ov.id}>
                  <Label className="mb-1 block">{ov.name}</Label>
                  <Select value={exportSelections[ov.id] || ""} onValueChange={(v) => setExportSelections({ ...exportSelections, [ov.id]: v })}>
                    <SelectTrigger>
                      <SelectValue placeholder="No Override (Default)" />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="none" className="text-muted-foreground">Skip Override</SelectItem>
                      {ov.options.map(opt => (
            ...
            {currentActivePreset && (
              <div className="mb-4 p-3 bg-green-500/10 border border-green-500/20 rounded-md flex items-center gap-2 text-sm text-green-500">
                ...
                <span>Active Preset: <strong>{currentActivePreset.name}</strong></span>
              </div>
            )}
            ...
            <div className="mt-6 pt-6 border-t border-border text-xs text-muted-foreground">
              <strong>Waterfall Resolution:</strong><br />
              1. Global Default (Lowest)<br />
          ...
          <div className="glass-panel p-6 rounded-xl animate-in fade-in slide-in-from-bottom-4">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-bold flex items-center"><Bookmark className="w-4 h-4 mr-2 text-primary" /> Saved Presets</h3>
              <DropdownMenu>
                ...
                <DropdownMenuContent align="center" className="w-[300px]">
                  <div className="px-2 py-1.5 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Other Projects</div>
                  <DropdownMenuSeparator />
                  {projects.filter(p => p.id !== id).length === 0 && (
                    <div className="p-2 text-xs text-muted-foreground text-center">No other projects found.</div>
                  )}
                  {projects.filter(p => p.id !== id).map(otherProj => {
                    const otherPresets = presets.filter(p => p.projectId === otherProj.id);
                    if (otherPresets.length === 0) return null;
                    return (
                      <div key={otherProj.id} className="mb-2">
                        <div className="px-2 py-1 text-xs text-primary/70 bg-primary/5">{otherProj.name}</div>
                        {otherPresets.map(op => {
                          ...
                          const isDup = projectPresets.some(p => {
                            ...
                            return pKeys.every(k => p.selections[k] === mappedSelections[k]);
                          });

                          return (
                            <DropdownMenuItem key={op.id} onSelect={(e) => {
            ...
            {projectPresets.length === 0 ? (
              <div className="text-sm text-muted-foreground p-4 text-center border border-dashed border-border rounded-lg bg-black/5">
                No presets saved yet.<br /><span className="text-xs">Generate a configuration to save one.</span>
              </div>
            ) : (
              <div className="space-y-2">
                {projectPresets.map(preset => (
                  <div key={preset.id} className="flex items-center justify-between p-3 rounded-lg bg-black/5 dark:bg-white/5 border border-border group hover:border-primary/50 transition-colors">
                    <button
                    ...
                    <Button variant="ghost" size="icon" className="opacity-0 group-hover:opacity-100 transition-opacity h-8 w-8 hover:bg-destructive/20 hover:text-destructive" onClick={() => deletePreset(preset.id)}>
                      <X className="w-4 h-4" />
                    </Button>
                  </div>
                ))}
        ...
        <div className="lg:col-span-8">
          ...
          {viewMode === "byOverride" && (
            <div className="mb-8">
              ...
              <Select value={selectedOverrideId} onValueChange={setSelectedOverrideId}>
                <SelectTrigger className="w-full md:w-[300px]">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  {overrides.map(ov => (
                    <SelectItem key={ov.id} value={ov.id}>{ov.name}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
