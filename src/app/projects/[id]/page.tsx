...
export default function ProjectSchemaBuilder() {
  ...
  if (!project) return <div className="p-12 text-center text-destructive">Project not found.</div>;

  const handleAddKey = () => {
    setKeys([...keys, { id: uuidv4(), name: "", description: "", overrides: [] }]);
  };
  ...
  const handleToggleOverride = (keyId: string, overrideId: string, checked: boolean) => {
    setKeys(keys.map(k => {
  ...
  const handleImportKeys = (importedKeys: SchemaKey[]) => {
    setKeys(prev => {
      ...
      importedKeys.forEach(importedKey => {
        const existingIndex = next.findIndex(k => k.name === importedKey.name);
        if (existingIndex >= 0) {
          ...
          next[existingIndex] = {
            ...next[existingIndex],
            description: importedKey.description || next[existingIndex].description,
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground">Define required ENV keys and attach overrides.</p>
          </div>
        </div>
        <div className="flex items-center gap-4">
          ...
          <Button onClick={handleSave} className="gap-2"><Save className="w-4 h-4" /> Save Schema</Button>
        </div>
      </header>
      ...
      <div className="space-y-6">
        {keys.length === 0 && (
          <div className="text-center p-12 glass-panel rounded-xl">
            ...
            <p className="text-muted-foreground mb-6">Start building your environment configuration from scratch or import from another project.</p>
            <div className="flex items-center justify-center gap-4">
              <Button onClick={handleAddKey}><Plus className="w-4 h-4 mr-2" /> Add Key Manually</Button>
              <Button variant="outline" onClick={() => setIsImportModalOpen(true)}><Import className="w-4 h-4 mr-2" /> Import Project</Button>
            </div>
          </div>
        )}

        {keys.map((keyObj) => (
          <div key={keyObj.id} className="glass-panel p-6 rounded-xl animate-in slide-in-from-bottom-2 flex flex-col md:flex-row gap-6 relative">
            <button
            ...
            <div className="flex-1 space-y-4">
              <div>
                ...
                <Input
                  value={keyObj.name}
                  onChange={e => handleUpdateKey(keyObj.id, { name: e.target.value.toUpperCase().replace(/\s+/g, '_') })}
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={e => handleUpdateKey(keyObj.id, { description: e.target.value })}
            ...
            <div className="flex-1 bg-black/10 dark:bg-white/5 p-4 rounded-lg">
              <Label className="mb-3 block text-muted-foreground">Attach Overrides</Label>
              {overrides.length === 0 ? (
                <p className="text-sm text-yellow-500">No global overrides defined yet.</p>
              ) : (
                <div className="space-y-3">
                  {overrides.map(ov => (
                    <div key={ov.id} className="flex items-center justify-between">
                      <Label htmlFor={`switch-${keyObj.id}-${ov.id}`} className="cursor-pointer font-normal">{ov.name}</Label>
                      <Switch
                        id={`switch-${keyObj.id}-${ov.id}`}
                        checked={keyObj.overrides.includes(ov.id)}
                        onCheckedChange={(c) => handleToggleOverride(keyObj.id, ov.id, c)}
                      />
                    </div>
                  ))}
                </div>
        ...
        {keys.length > 0 && (
          <Button variant="outline" onClick={handleAddKey} className="w-full py-8 border-dashed glass-panel">
